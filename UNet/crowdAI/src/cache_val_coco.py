...
2. Caches val/test data for future trainings.
...
def cache_val_16():
    ...
    dataset_val.load_dataset(dataset_dir=os.path.join("data", "val"), load_small=True)
    dataset_val.prepare()
    image_ids=np.copy(dataset_val.image_ids)
    
    
    num_val = len(image_ids)
    img = dataset_val.load_image(image_ids[0])
    ...
    f = h5py.File(os.path.join(data_path, 'val_coco.h5'), 'w')
    imgs = f.create_dataset('val', (num_val, num_channels, image_rows, image_cols), dtype=np.float16)

    imgs_mask = f.create_dataset('val_mask_coco', (num_val, num_mask_channels, image_rows, image_cols), dtype=np.uint8)
    ...
    f['val_ids'] = np.array(ids).astype('|S9')
